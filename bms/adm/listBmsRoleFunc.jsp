...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			{
				var url = "${pageContext.request.contextPath}/bms/adm/bmsrolefunc/beforeDeleteBmsRoleFunc.do";
				if(!$chk(id))
				{
			...
			{
				var url = "${pageContext.request.contextPath}/bms/adm/bmsrolefunc/beforeAddBmsRoleFunc.do";
		...
		<ec:table tableId="BmsRoleFuncExList" items="list" var="m" action=""
			...
			<ec:row>
				<ec:column title="PK" property="id" headerCell="selectAll" alias="ckids">
					...
					<a
						href="${pageContext.request.contextPath}/bms/adm/bmsrolefunc/detailBmsRoleFunc.do?id=${m.id}">
						${m.id} </a>
				</ec:column>
				...
				<ec:column title="操作" property="EEE" sortable="false"
					filterable="false">
					<a
						href="${pageContext.request.contextPath}/bms/adm/bmsrolefunc/beforeEditBmsRoleFunc.do?id=${m.id}">编辑</a>
					<a
